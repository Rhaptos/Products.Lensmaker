...
Refresh.Web.DefaultColorPickerSettings = {
	startMode:'h',
	startHex:'ff0000',
	clientFilesPath: portal_url+'/colorchooser-resources/images/'
};
...
Refresh.Web.ColorPicker.prototype = {
	...
	initialize: function(parent, settings) {
		...
		this.settings = Object.extend(Object.extend({},Refresh.Web.DefaultColorPickerSettings), settings || {});
                //IE needs to validate the start color when setting a background color (See #9813)
                //  otherwise it raises an exception.
                if (!this.settings.startHex.match(/^[0-9a-fA-F]{6}$/)) {
                  this.settings.startHex = Refresh.Web.DefaultColorPickerSettings.startHex;
                }
		 ...
		 ************************************ */
		function addId(o){
			var id = Ext.id(null, 'colorchooser-');
			o.id = id;
			return o;
		};

		var colorMap = addId({ tag:'div', style:'width:256px;'});
		var colorBar = addId({ tag:'div'});
		var orig = addId({ tag:'div', style:'vertical-align:middle; text-align:center; width:60px; height:30px;', children:[ 'current' ]});
		var preview = addId({ tag:'div', style:'vertical-align:middle; text-align:center; width:60px; height:30px;', children:[ 'new' ]});

		/* Build the radio buttons */
		var modeGroup = Ext.id(null, 'color-mode-');
		function buildRadio(optionId, title, letter, trailing) {
			var radio = addId({ tag:'input',  type:'radio', name:modeGroup, value:optionId, cls:'noborder' });
			var label = addId({ tag:'label', forHtml:radio.id, title:title, html:letter+':'});
			var input = addId({ tag:'input', type:'text', value:'0', style:'width: 40px;'});
			var tr = { tag:'tr', children:[
						{ tag:'td', children:[ radio ]},
						{ tag:'td', children:[ label ]},
						{ tag:'td', children:[ input, trailing ]}
					]};
			return {radio:radio, label:label, input:input, tr:tr};
		}

		this.ids = {};
		this.ids.hue = buildRadio('0', 'Hue', 'H', '\u00b0');
		this.ids.sat = buildRadio('1', 'Saturation', 'S', '%');
		this.ids.bright = buildRadio('2', 'Brightness', 'B', '%');
		this.ids.red = buildRadio('r', 'Red', 'R', '');
		this.ids.green = buildRadio('g', 'Green', 'G', '');
		this.ids.blue = buildRadio('b', 'Blue', 'B', '');

		this.ids.hex = addId({ tag:'input',  type:'text', value:'FF0000', style:'width: 60px;'});

		var htmlScaffolding = { tag:'table', children:[
			{ tag:'tr', children:[
				{ tag:'td', valign:'top', children:[ colorMap ]},
				{ tag:'td', valign:'top', children:[ colorBar ]},
				{ tag:'td', valign:'top', children:[
					{ tag:'table', children:[
						{ tag:'tr', children:[
							{ tag:'td', style:'margin:0;padding:0;', colspan:'3', children:[ orig ]}
						]},
						{ tag:'tr', children:[
							{ tag:'td', style:'margin:0;padding:0;', colspan:'3', children:[ preview ]}
						]},
						this.ids.hue.tr, this.ids.sat.tr, this.ids.bright.tr,
						{ tag:'tr', children:[
							{ tag:'td', colspan:'3', height:'5'}
						]},
						this.ids.red.tr, this.ids.green.tr, this.ids.blue.tr,
						{ tag:'tr', children:[
							{ tag:'td', html:'#:'},
							{ tag:'td', colspan:'2', children:[ this.ids.hex ]}
						]}
					]}
				]}
			]}
		...
		Ext.DomHelper.append(Ext.id(parent), htmlScaffolding);
		
		
		// attach radio & check boxes
		this._hueRadio = $(this.ids.hue.radio.id);
		this._saturationRadio = $(this.ids.sat.radio.id);
		this._valueRadio = $(this.ids.bright.radio.id);
		
		this._redRadio = $(this.ids.red.radio.id);
		this._greenRadio = $(this.ids.green.radio.id);
		this._blueRadio = $(this.ids.blue.radio.id);
		...
		Refresh.Web.ColorPicker.extjsUpdatePreview(this.settings.startHex, orig.id);
		this._preview = $(preview.id);
		
		
		// MAP
		this._mapBase = $(colorMap.id);
		...
		// attach map slider
		this._map = new Refresh.Web.Slider(this._mapL2, {parent:this._mapBase, xMaxValue: 255, yMinValue: 255, arrowImage: this.settings.clientFilesPath + 'mappoint.gif'});

		// attach color slider
		this._slider = new Refresh.Web.Slider(this._barL4, {parent:this._bar, xMinValue: 1,xMaxValue: 1, yMinValue: 255, arrowImage: this.settings.clientFilesPath + 'rangearrows.gif'});;

		// attach color values
		this._cvp = new Refresh.Web.ColorValuePicker(this.ids);
	...
	updatePreview: function() {
		try {
			Refresh.Web.ColorPicker.extjsUpdatePreview(this._cvp.color.hex, this._preview);
		} catch (e) {}
	},
 ...
 * opening a colorpicker window
 ********************************** */
Refresh.Web.ColorPicker.extjsPopupHelper = function(input) {

    //TODO Create defaults for the title and buttons...
    var input = Ext.get(input);

    // create the window on the first click and reuse on subsequent clicks
    if(input && !input._win){
    
        input._win = new Ext.Window({
            layout:'fit',
            width:450,
            height:334,
            closeAction:'hide',
            ...
            buttons: [{
                ...
                handler: function(){
                    if(input._cp.color) {
                        var background = input._cp.color.hex;
                        input.dom.value = background;
                        if(input.dom.onchange) {
                            input.dom.onchange(input.dom);
                        }
                    }
                    input._win.hide();
                }
        ...
        input._win.on('show', function() {
            // Just delete the old colorpicker. The cursors seem to disappear otherwise
            if(input._cp != null) {
                input._win.body.clean();
                var child;
                while(child = input._win.body.dom.firstChild) {
                    Ext.get(child).remove();
                }
            }
            input._cp = new Refresh.Web.ColorPicker(input._win.body, {startHex: input.dom.value, startMode:'h'});
        });
        input._win.on('move', function() {
            //update the positions of the cursors
            // They don't get updated.
            input._cp.show();
        });
        input._win.on('hide', function() {
            input._cp.hide();
        });
    }
    
    input._win.show(this);
    input._cp.show();
};

Refresh.Web.ColorPicker.extjsUpdatePreview = function(bgHexCode, preview) {
  preview = Ext.get(preview);
  if(!bgHexCode || bgHexCode == '') {
    preview.hide();
  } else {
    var foreground = findBestTextColor(bgHexCode);
    preview.setStyle('background-color', '#'+bgHexCode);
    preview.setStyle('color', '#'+foreground);
    preview.show();
  }
}
